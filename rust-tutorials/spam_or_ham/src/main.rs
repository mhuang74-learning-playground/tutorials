...
fn parse(data: &str) -> (SparseRowArray, Array) {
	...
	for (row_num, line) in data.lines().enumerate() {
		...
		// Lets split thel ine in two here.

		let (label, text) = line.split_at(line.find('\t').unwrap());

		// Conver the labels to binary. Lets use pattern matching
		// to ensure that the program is aborted if an unexpected 
		// label is uncountered.
...
fn fit(X: &SparseRowArray, y: &Array) -> (f32, f32) {
	...
	// The cross validation iterator returns indices for train and test rows
	...
	for (train_indices, test_indices) in CrossValidation::new(y.rows(), num_folds) {
		...
		// Repeated calls to `fit` perform epocs of training
...
fn main() {
	...
	let zipped = download("http://archive.ics.uci.edu/ml/machine-learning-databases/00228/smsspamcollection.zip");

    println!("Downloaded {} bytes of data", zipped.len());

    let raw_data = unzip(zipped);

    for line in raw_data.lines().take(10) {
    	println!("{}", line);
    }
